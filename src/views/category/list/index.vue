...
const handleCategoryState = (row) => {
  const actionText = row.state === 1 ? "禁用" : "启用"
  if (row.state === 1) {
    ElMessageBox.confirm(`正在${actionText}分类：${row.categoryName}，确认${actionText}？`, "提示", {
      ...
      updateCategoryStateApi({ categoryId: row.categoryId, state: 0 })
        .then(() => {
    ...
    updateCategoryStateApi({ categoryId: row.categoryId, state: 1 })
      .then(() => {
        ElMessage.success(`${actionText}成功`)
        getCategoryData()
      })
        ...
        <el-table :data="tableData" @selection-change="handleSelectionChange">
          ...
          <el-table-column fixed="right" label="操作" width="180" align="center">
            <template #default="scope">
              ...
              <el-button :type="scope.row.state === 1 ? 'danger' : 'success'" text bg size="small"
